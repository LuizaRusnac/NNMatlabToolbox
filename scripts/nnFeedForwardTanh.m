function [ y, z ] = nnFeedForwardTanh( wpack,X,nin,nlayer,nhidden,nout)
% -------------------------------------------------------------------------
% NNFEEDFORWARD calculeaza iesirea din retea
% 
% Descriere
% 
% [y, z] = NNFEEDFORWARDTANH(WPACK, X, NIN, NLAYER, NHIDDEN, NOUT) calculeaza
...
start = 1;
stop = v(1,1)*v(2,1);
y{1,1} = round(10000.*tanhFnc(inputBias(X)*reshape(wpack(start:stop),v(1,1),v(2,1))'))./10000;
...
for i=2:nlayer+1
    ...
    if  i!= nlayer+1
      y{i,1} = round(10e6.*tanhFnc(inputBias(y{i-1,1})*reshape(wpack(start:stop),v(1,i),v(2,i))'))./10e6;
    else
      y{i,1} = round(10e6.*softmax(inputBias(y{i-1,1})*reshape(wpack(start:stop),v(1,i),v(2,i))'))./10e6;
